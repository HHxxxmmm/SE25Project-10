...
public class PaymentServiceImpl implements PaymentService {
    ...
    @Override
    @Transactional
    public BookingResponse payOrder(Long orderId, Long userId) {
        // 优先从Redis查询订单
        Optional<Order> orderOpt = redisService.getCachedOrder(orderId.toString());
        if (!orderOpt.isPresent()) {
            // Redis中没有，从数据库查询
            orderOpt = orderRepository.findByOrderIdAndUserId(orderId, userId);
            if (!orderOpt.isPresent()) {
        ...
        System.out.println("支付成功: " + order.getOrderNumber() + ", 订单ID: " + order.getOrderId());
        
        return BookingResponse.successWithMessage("支付成功", order.getOrderNumber(), order.getOrderId(), order.getTotalAmount(), LocalDateTime.now());
    }
